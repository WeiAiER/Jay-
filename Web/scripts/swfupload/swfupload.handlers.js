$(function () {
    ...
    $.fn.OInitSWFUpload = function (p) {
        ...
        var swfu = new SWFUpload({
            ...
            upload_progress_handler: uploadProgress,
            upload_error_handler: uploadError,
            upload_success_handler: OuploadSuccess,
            upload_complete_handler: uploadComplete,
...
function OuploadSuccess(file, serverData) {
    try {
        var jsonstr = eval('(' + serverData + ')');
        if (jsonstr.status == '0') {
            var progressObj = $(this.customSettings.upload_target).children(".upload-progress");
        ...
        if (jsonstr.status == '1') {
            ...
            if (this.customSettings.button_action == SWFUpload.BUTTON_ACTION.SELECT_FILE) {
                ...
                OaddImage($(this.customSettings.upload_target), jsonstr.path, jsonstr.thumb);
            }
            var progressObj = $(this.customSettings.upload_target).children(".upload-progress");
            progressObj.children(".txt").html("上传成功：" + file.name);
        }
...
function OaddImage(targetObj, originalSrc, thumbSrc) {
    //插入到相册UL里面
    var newLi = $('<li>'
    + '<input type="hidden" name="ohid_photo_name" value="0|' + originalSrc + '|' + thumbSrc + '" />'
    + '<input type="hidden" name="ohid_photo_remark" value="" />'
    + '<div class="img-box" onclick="setFocusImg(this);">'
    + '<img src="' + thumbSrc + '" bigsrc="' + originalSrc + '" />'
    + '<span class="remark"><i>暂无描述...</i></span>'
    + '</div>'
    + '<a href="javascript:;" onclick="osetRemark(this);">描述</a>'
    + '<a href="javascript:;" onclick="delImg(this);">删除</a>'
    + '</li>');
    newLi.appendTo(targetObj.siblings(".photo-list").children("ul"));
...
function osetRemark(obj) {
    var parentObj = $(obj); //父对象
    var hidRemarkObj = parentObj.prevAll("input[name='ohid_photo_remark']").eq(0); //取得隐藏值
    var m = $.dialog({
        lock: true,
        max: false,
        min: false,
        ...
        button: [{
            ...
            callback: function () {
                ...
                parentObj.parent().parent().find("li input[name='ohid_photo_remark']").val(remarkObj.val());
                parentObj.parent().parent().find("li .img-box .remark i").html(remarkObj.val());
            }
        }, {
            name: '单张描述',
...
//删除图片LI节点、数据库数据
function delImgdb(obj,id) {
    var parentObj = $(obj).parent().parent();
    var focusPhotoObj = parentObj.parent().siblings(".focus-photo");
    var smallImg = $(obj).siblings(".img-box").children("img").attr("src");
    $(obj).parent().remove(); //删除的LI节点
    $.getJSON(
            "tools/DelPic.ashx?type=" + id,
            null,
            function (data){
               alert("删除图片成功！");      
        });
